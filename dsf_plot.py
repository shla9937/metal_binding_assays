...
def plot_tm_scatter(tm_df):
    ...
    for metal, wells in metals.items():
        ...
        if len(filtered_tm_values) >= 4:  # Ensure at least 4 data points for fitting
            ...
            try:
                if wt_avg_tm is not None:
                    # Determine the direction of the binding curve
                    if max(filtered_tm_values) > wt_avg_tm:  # Binding goes up
                        def hill_eq_fixed(concentration, ymax, K, n):
                            return hill_eq(concentration, ymax=ymax, K=K, n=n, ymin=wt_avg_tm)
                        initial_guess = [max(filtered_tm_values), np.median(filtered_concentrations), 1.0]
                    else:  # Binding goes down
                        def hill_eq_fixed(concentration, ymin, K, n):
                            return hill_eq(concentration, ymax=wt_avg_tm, K=K, n=n, ymin=ymin)
                        initial_guess = [min(filtered_tm_values), np.median(filtered_concentrations), 1.0]
                    ...
                    popt, _ = curve_fit(
                        ...
                        filtered_tm_values,
                        p0=initial_guess  # Initial guesses
                    )

                    # Extract parameters
                    if max(filtered_tm_values) > wt_avg_tm:
                        ymax, K, n = popt
                        ymin = wt_avg_tm
                    else:
                        ymin, K, n = popt
                    ...
                    fit_y = hill_eq(fit_x, ymax=ymax, ymin=ymin, K=K, n=n)
                    ax.plot(fit_x, fit_y, color='gray')  # No legend entry for fit line
                    kd_summary.append(f"{metal}: Kd = {K:.2f} µM")
                else:
                    print(f"WT Avg Tm is not defined for {metal}, skipping fit.")
            except RuntimeError:
                print(f"Could not fit Hill equation for {metal}")
        else:
            print(f"Not enough data points to fit Hill equation for {metal}")

    # Plot WT and EDTA lines
    if wt_avg_tm is not None:
        ax.axhline(wt_avg_tm, color='gray', linestyle='-', label=f"WT Avg Tm = {wt_avg_tm:.2f} °C")
    if edta_avg_tm is not None:
        ax.axhline(edta_avg_tm, color='black', linestyle='-', label=f"EDTA Avg Tm = {edta_avg_tm:.2f} °C")

    # Move legend and add Kd summary box
    ax.set_xlabel("Concentration (µM)")
    ax.set_ylabel("Tm (°C)")
    ax.set_title("Tm Scatter Plot with Hill Equation Fit")

    summary_text = "\n".join(kd_summary)
    ax.text(1.05, 0.5, summary_text, transform=ax.transAxes, fontsize=10, verticalalignment='center', bbox=dict(boxstyle="round", facecolor="white", edgecolor="black"))

...
def main():
    ...
    parser.add_argument('-c', '--csv', type=str, required=True, help="List of csvs from 01_dsf_import.py")
    parser.add_argument('-t', '--title', type=str, required=True, help="Title")
    args = parser.parse_args()

    df = parse_csv_file(args.csv)
    metal_df = plot_384well_data(df, os.path.dirname(args.csv))
    print(metal_df)
    # out_csv = os.path.join(os.path.dirname(args.csv), "384well_tm_values.csv")
    ...
    # metal_df = plot_tm_scatter(metal_df, args.title, output_dir)
...
def metals_and_concentrations():
    ...
    for i, ctrl in enumerate(['WT', 'EDTA']):
        for j in range(6):
            well = f"P{13 + i*6 + j}"
